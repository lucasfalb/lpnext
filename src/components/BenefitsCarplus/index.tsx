...
const SelectableItem: React.FC<SelectableItemProps> = ({ id, description, selected, onSelect }) => {
  const isSelected = id === selected;
  const itemClass = `p-5 relative w-full no-select cursor-pointer rounded ${isSelected ? 'text-black bg-white' : 'text-gray bg-whiteSmoke'}`;

  return (
    <div className={itemClass} onClick={() => onSelect(id)}>
      {description}
  ...
  return (
    <Container>
      ...
      <Container className="px-0 col-span-full">
        <aside className="flex flex-col gap-6 relative">
